    ...
    public static function getPreviewImage($om, $oids) {
        ...
        foreach($res as $oid => $odoc) {
            ...
            try {
                ...
                if(($old_width/$old_height)<($target_width/$target_height)){
                    $new_height = $old_height * $target_width / $old_width;
                    $new_width = $target_width;
                }
                else{
                    $new_height = $target_height;
                    $new_width = $old_width * $target_height/$old_height;
                }

                $min = min($old_width, $old_height);

                $offset_x  = round(($old_width - $min)/2);
                $offset_y  = round(($old_height - $min)/2);
                ...
                imagecopyresized($newImage, $oldImage, 0, 0, $offset_x, $offset_y, $new_width, $new_height, $old_width, $old_height);
