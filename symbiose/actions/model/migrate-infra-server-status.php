...
[$params, $providers] = eQual::announce([
    'description' => 'Backfills resource status models from their server or instance target.',
...
foreach(['model', 'server_id', 'instance_id'] as $column) {
...
$invalid_statuses = $fetch_count(
    "SELECT COUNT(*) AS row_count
     FROM `{$table}`
     WHERE (`server_id` IS NULL AND `instance_id` IS NULL)
        OR (`server_id` IS NOT NULL AND `instance_id` IS NOT NULL);"
);
if($invalid_statuses > 0) {
    throw new Exception('invalid_status_target', EQ_ERROR_CONFLICT_OBJECT);
}

$server_status = "CONVERT(0x" . bin2hex('infra\server\ServerStatus') . " USING utf8mb4)";
$instance_status = "CONVERT(0x" . bin2hex('infra\server\InstanceStatus') . " USING utf8mb4)";
...
try {
    $db->sendQuery(
        "UPDATE `{$table}`
         SET `model` = CASE
            WHEN `server_id` IS NOT NULL THEN {$server_status}
