...
[$params, $providers] = eQual::announce([
    'description' => 'Backfills payment models from order payment references.',
...
foreach(['model', 'order_payment_id'] as $column) {
    if(!isset($columns[$column])) {
        throw new Exception("Missing required column '{$table}.{$column}'.", EQ_ERROR_INVALID_CONFIG);
    }
}

$order_payment_part = "CONVERT(0x" . bin2hex('sale\pos\OrderPaymentPart') . " USING utf8mb4)";
$payment = "CONVERT(0x" . bin2hex('sale\pay\Payment') . " USING utf8mb4)";
...
try {
    $db->sendQuery(
        "UPDATE `{$table}`
         SET `model` = CASE
            WHEN `order_payment_id` IS NOT NULL THEN {$order_payment_part}
            ELSE {$payment}
         END;"
    );
    $db->sendQuery('COMMIT;');
