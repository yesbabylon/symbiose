...
class UserStatus extends Model
{
	...
	public static function getColumns(): array
	{
		return [
			'course_id' => [
				...
				'required'       => true,
				'ondelete'       => 'cascade'
				// delete status when parent course is deleted
			],
			...
			'module_id' => [
				...
				'required'       => true,
				'ondelete'       => 'cascade'
				// delete status when parent module is deleted
			],
			...
			'page_count' => [
				'type'        => 'integer',
				'description' => 'Number of pages within the chapter.',
				'default'     => 1
			],
	...
	public static function onupdateIsComplete($orm, $oids, $values, $lang)
	{
		...
		$statuses = $orm->read(__CLASS__, $oids, [
			'course_id',
			'user_id'
		], $lang);
		...
		if ($statuses && count($statuses)) {
			foreach ($statuses as $oid => $status) {
				...
				$ids = $orm->search('learn\UserAccess', [
					['user_id', '=', $user_id],
					['course_id', '=', $course_id]
				]);
				if ($ids && count($ids)) {
