...
$module = Module::id($params['module_id'])->read(['order', 'course_id', 'chapter_count', 'chapters_ids'])->first();
...
if (count($status_ids) < 1) {
	...
	$status = UserStatus::ids($status_ids)->read([
		'course_id',
		'chapter_index',
		'page_index',
		'page_count',
	...
	if (($params['chapter_index'] == $status['chapter_index'] && $params['page_index'] == $status['page_index'] + 1) || ($params['chapter_index'] == $status['chapter_index'] + 1 && $params['page_index'] == 0)) {
	...
	if ($params['chapter_index'] + 1 >= $module['chapter_count'] && $params['page_index'] + 1 >= $chapter['page_count']) {
		...
		if (!$status['is_complete']) {
			...
			// Add a new userStatus entry for the next module if there is one
			$next_module_id = Module::search([
				['course_id', '=', $module['course_id']],
				['order', '=', $module['order'] + 1]
			])->ids()[0];

			// get the page count of the next chapter
			$page_count = 1;
			if ($next_module_id) {
				$nextChapter = Chapter::search([
			...
			$access = UserAccess::search([
				['course_id', '=', $status['course_id']],
				['user_id', '=', $user_id]
			])->read(['is_complete'])->first();
...
$context->httpResponse()->status(204)->send();