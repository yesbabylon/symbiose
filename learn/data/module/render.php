...
$module = eQual::run('get', 'learn_module', ['lang' => $params['lang'], 'id' => $params['id']]);
          ...
          foreach($group['widgets'] as $widget_index => $widget) {
            if(in_array($widget['type'], ['image_popup', 'selector_popup']) && $widget['on_click'] == 'image_full()') {
                ...
                $new_page = [
                  'leaves' => [
                      [
                          'background_image'   => learn_module_render_adapt_url($widget['image_url']),
...
function displayPage($page) {
  ...
  foreach($page['leaves'] as $leave) {
    ...
    if(isset($leave['background_image']) && strlen($leave['background_image'])) {
      $background_img = learn_module_render_adapt_url($leave['background_image']);
    }
        ...
        switch($widget['type']) {
          ...
          case 'selector_section':
            ...
            if(isset($widget['image_url']) && strlen($widget['image_url'])) {
              $background_img = learn_module_render_adapt_url($widget['image_url']);
            }
          ...
          case 'video':
            ...
            $content = "<div class=\"video-container\"><a href=\"". learn_module_render_adapt_url($widget["video_url"]) ."\"><div class=\"play\"></div></a></div>";
            break;
          case 'image_popup':
            $content = "<div class=\"image-container\"><img width=\"100%\" height=\"100%\" src=" . learn_module_render_adapt_url($widget["image_url"]) . "></div> ";
            break;
          case 'selection':
...
function learn_module_render_adapt_url($url) {
  if(substr($url, 0, 1) == '/') {
