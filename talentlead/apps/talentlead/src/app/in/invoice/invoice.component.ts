...
export class InvoiceComponent implements OnInit, AfterContentInit {
            ...
            if(params) {
                try {
                    ...
                    if(params.hasOwnProperty('service_id')){
                        this.service_id = <number> parseInt(params['service_id'], 10);
    ...
    private async loadService() {
        const result:Array<any> = <Array<any>> await this.api.read("yesbabylon\\service\\Service", [this.service_id], Object.getOwnPropertyNames(new Service()));
        if(result && result.length) {
            const item:any = result[0];
            let service:any = new Service();
            for(let field of Object.getOwnPropertyNames(service) ) {
                if(item.hasOwnProperty(field)) {
                    service[field] = item[field];
                }
            }
            ...
            // if(this.service.contacts_ids && this.service.contacts_ids.length) {
    ...
    private async loadCustomer() {
        const result = <Array<any>> await this.api.read("sale\\customer\\Customer", [this.service.customer_id], Object.getOwnPropertyNames(new Customer()));
        if(result && result.length) {
            const item:any = result[0];
    ...
    private async refreshRecipientAddresses() {
        console.debug('BookingInvoiceComponent::refreshRecipientAddresses', this.customer, this.contacts);
    ...
    public async onSend() {
        ...
        */
        ...
        try {
            this.loading = true;
            const response:any = await this.api.call('?do=yesbabylon_send-invoice', {
    ...
    public onclickCustomer() {
        let descriptor:any = {
            ...
            context: {
                ...
                type: 'form',
                name: 'default',
                domain: ['id', '=', this.service.customer_id],
                mode: 'view',
                purpose: 'view',
                display_mode: 'popup',
                callback: (data:any) => {
